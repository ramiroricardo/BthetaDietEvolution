...
chmod u+x breseq_script_D84_prokka_withMQpara_v3ref.sh

# run script (this is set up to run multiple samples in parallel)
nohup bash breseq_script_D84_prokka_withMQpara_v3ref.sh &
...
Plots for the chromosome showed that any 1000bp region with more than 3 mutations is beyond the 99% CI, so beelow I filter for those gene regions that have 3 or more mutations
...
d84_long_parallel<-d84_long_annot[c(1:15)] %>% subset(gene %in% parallel_genes_woutMutators)
...
- Mutational targets enriched for a single diet if the mutational target is mutated in a significantly higher number of mice in that diet, relative to the other two (emmeans p-value<0.1; e.g. mutations in BT_4247 are significantly more prevalent in WD then either SD or AD, but there is no difference between SD and AD)
- Mutational targets enriched for a two diets if the mutational target is mutated in a significantly less (emmeans p-value<0.1) in one diet relative to the other two and there is no significant difference between the later pair (e.g. mutations in BT_0370 are significantly less prevalent in WD relative to AD or SD, but there was no difference between AD and SD)  

However, there were three mutational targets in which only one or none of the diet pairwise comparisons was significant. In this cases, these were attributed to enriched groups as follows:
- BT_1754 (TDA1000_04278): only significant in the diet main effect (before fdr). As the mutations in this gene are only detected in the SD group, it was labeled as SD enriched
- BT_1725/BT_1726 (TDA1000_04249/TDA1000_04250): only significant in the WD - AD comparison, with 6 mice mutated in WD, 3 in SD and 0 in AD. As mutations in WD occur in twice as many mice as SD, we decided to label this as WD enriched   
- BT_1657 (TDA1000_04178):  only significant in the SD - WD comparison, with 6 mice mutated in WD, 1 in SD and 2 in AD. As mutations in WD occur in twice as many mice as SD+AD, we decided to label this as WD enriched 
...
df_dietDifferentialMutationalTargets_fdr_p0.05$sig_SDAD<-ifelse(df_dietDifferentialMutationalTargets_fdr_p0.05$`SD - AD`<=0.1,1,0)
df_dietDifferentialMutationalTargets_fdr_p0.05$sig_SDWD<-ifelse(df_dietDifferentialMutationalTargets_fdr_p0.05$`SD - WD`<=0.1,1,0)
df_dietDifferentialMutationalTargets_fdr_p0.05$sig_WDAD<-ifelse(df_dietDifferentialMutationalTargets_fdr_p0.05$`WD - AD`<=0.1,1,0)
...
df_dietDifferentialMutationalTargets_fdr_p0.05$group_enriched<-c("AD","AD","WD&AD","AD","WD","WD","AD","SD&AD",
                                                                 "AD","AD","SD&AD","AD","WD","WD","SD")
