...
df_dietDifferentialMutationalTargets_fdr_p0.05<-df_dietDifferentialMutationalTargets_fdr_p0.05 %>% rename_at(15, ~paste0(., "_1"))
...
df_dietDifferentialMutationalTargets_fdr_p0.05<-df_dietDifferentialMutationalTargets_fdr_p0.05 %>% rename_at(16, ~paste0(., "_2"))
...
here, we testesd whether some mutations (of each specific gene) were differentially pervalent.  
...
ggplot(d84_long_parallel_prev_count,aes(x=annotation,y=count,fill=diet))+
...
ggplot(d84_long_parallel_prev[d84_long_parallel_prev$frequency>0,],aes(x=annotation,y=frequency,fill=diet))+
...
gene<-i
if(berryFunctions::is.error(assign(paste0("mod","_",i),glm(presenceAbsence~diet*seqid_position_annotation_gene,data=dat, family=binomial(),method = "brglmFit")))){
vals<-data.frame(cbind(gene,df=NA,Deviance=NA,resid_df=NA,resid_dev=NA,P.val_dietMutation=NA))  
} else{
assign(paste0("mod","_",i),glm(presenceAbsence~diet*seqid_position_annotation_gene,data=dat, family=binomial(),method = "brglmFit"))
mod<-get(paste0("mod","_",i))
moda<-anova(mod,test="LRT")
...
#interactive/downloadable table
DT::datatable(data = left_join(df_presAbs_gene,d84_long_parallel_prev[5:6] %>% distinct()), 
